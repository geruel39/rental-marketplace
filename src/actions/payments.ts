...
function isCompletedHitPayStatus(status: string | null | undefined) {
  return status?.trim().toLowerCase() === "completed";
}

function getReconciledPaymentAmount(params: {
  amount: number | null;
  checkout?: CheckoutTransaction | null;
}) {
  if (typeof params.amount === "number" && Number.isFinite(params.amount)) {
    return roundMoney(params.amount);
  }

  if (params.checkout) {
    const checkoutGrossAmount =
      typeof params.checkout.gross_amount === "number"
        ? params.checkout.gross_amount
        : Number(params.checkout.gross_amount);

    if (Number.isFinite(checkoutGrossAmount)) {
      return roundMoney(checkoutGrossAmount);
    }
  }
...
export async function reconcileCheckoutPayment(
  ...
  try {
    const checkout = await getCheckoutTransaction(checkoutId);
    if (!checkout?.hitpay_payment_request_id) {
      return checkout?.booking_id ?? null;
    }
    ...
    const payment = await getPaymentStatus(checkout.hitpay_payment_request_id);
    if (!isCompletedHitPayStatus(payment.status)) {
      return null;
    }

    const amount = getReconciledPaymentAmount({
      amount: payment.amount,
      checkout,
    });
    if (amount === null) {
      console.error("[PAYMENTS] Could not determine checkout payment amount:", checkoutId);
      return null;
    }

    return await handleCompletedCheckoutPayment({
      checkoutId,
      hitpayPaymentId:
        payment.paymentId ?? checkout.hitpay_payment_id ?? checkout.hitpay_payment_request_id,
      hitpayPaymentRequestId:
        payment.paymentRequestId ?? checkout.hitpay_payment_request_id,
      amount,
      currency: payment.currency ?? checkout.currency ?? "SGD",
    });
  } catch (error) {
    console.error("[PAYMENTS] reconcileCheckoutPayment failed:", error);
    return null;
  }
}

export async function reconcilePendingBookingPayment(
  bookingId: string,
): Promise<boolean> {
  try {
    ...
    if (!booking.hitpay_payment_request_id) {
      return false;
    }
    ...
    const amountRaw =
      typeof payment.amount === "number" && Number.isFinite(payment.amount)
        ? payment.amount
        : typeof booking.net_collected === "number" && Number.isFinite(booking.net_collected)
          ? booking.net_collected
          : typeof booking.total_price === "number" && Number.isFinite(booking.total_price)
            ? booking.total_price
            : null;
    if (amountRaw === null) {
      console.error("[PAYMENTS] Could not determine booking payment amount:", bookingId);
      return false;
    }

    await handlePaymentConfirmed({
      hitpayPaymentId:
        payment.paymentId ?? booking.hitpay_payment_id ?? booking.hitpay_payment_request_id,
      hitpayPaymentRequestId:
        payment.paymentRequestId ?? booking.hitpay_payment_request_id,
      bookingId,
      amount: roundMoney(amountRaw),
      currency: payment.currency ?? "SGD",
    });

    const refreshed = await getBookingWithRelations(createAdminClient(), bookingId);
    return Boolean(
      refreshed.paid_at && refreshed.hitpay_payment_status === "completed",
    );
  } catch (error) {
    console.error("[PAYMENTS] reconcilePendingBookingPayment failed:", error);
    return false;
  }
}

export async function reconcileOutstandingCheckoutPaymentsForUser(params: {
  role: "lister" | "renter";
  userId: string;
}) {
  try {
    const admin = createAdminClient();
    const userColumn = params.role === "renter" ? "renter_id" : "lister_id";
    const { data, error } = await admin
      .from("transactions")
      .select("id")
      .eq(userColumn, params.userId)
    ...
    for (const row of data ?? []) {
      if (typeof row.id === "string") {
        await reconcileCheckoutPayment(row.id);
      }
    }
  } catch (error) {
    console.error("[PAYMENTS] reconcileOutstandingCheckoutPaymentsForUser failed:", error);
  }
}
